...
class Game 
  ...
  def initialize(new_name)
	...
	def history
		...
		if @record[@new_name].nil?
			@record[@new_name] = @count
		  json = File.read(@@json[@@gamenum - 1])
		  File.open(@@json[@@gamenum - 1], 'w') do |f|
		    f.puts JSON.pretty_generate(JSON.parse(json).merge(@record))
		  end
	   elsif @record.key?(@new_name) && @record[@new_name] > @count
	 	   @record[@new_name] = @count 
		   json = File.read(@@json[@@gamenum - 1])
		   File.open(@@json[@@gamenum - 1], 'w') do |f|
		     f.puts JSON.pretty_generate(JSON.parse(json).merge(@record))
